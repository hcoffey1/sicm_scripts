...
def parse_map_file(map_file, site_map, map_key, page_map=None,
  ...
  for line in mapf:
    ...
    if page_map:
      ...
      update_page_map(page_map, pg_addr, ddr, pmm)

    if huge_page_map:
      pg_addr = ( (paddr << addr_shift) >> HUGE_PAGE_SHIFT )
      update_page_map(huge_page_map, pg_addr, ddr, pmm)

    if gb_page_map:
      pg_addr = ( (paddr << addr_shift) >> GB_PAGE_SHIFT )
      update_page_map(gb_page_map, pg_addr, ddr, pmm)
...
def get_site_map(bench, size, config, args, it):
  pkl_file = get_site_map_pickle(args.bench, args.size, args.config,
             args.args, args.iter)
  if os.path.exists(pkl_file) and not args.clean:
    pklf = open(pkl_file, 'rb')
    site_map = pickle.load(pklf)
    pklf.close()
  else:
    print("rebuilding site map ... ", end='', flush=True)
    build_site_map(args.bench, args.size, args.config,
                   args.args, args.iter)
    pklf = open(pkl_file, 'rb')
    site_map = pickle.load(pklf)
    pklf.close()
...
def get_huge_page_map(bench, size, config, args, it):
  pkl_file = get_huge_page_map_pickle(bench, size, config, args, it)
  if os.path.exists(pkl_file) and not clean:
    pklf = open(pkl_file, 'rb')
...
def get_gb_page_map(bench, size, config, args, it):
  pkl_file = get_gb_page_map_pickle(bench, size, config, args, it)
  if os.path.exists(pkl_file) and not args.clean:
    pklf = open(pkl_file, 'rb')
